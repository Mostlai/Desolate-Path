...
const translationMap = {
    ...
    'Skeleton Swordsmaster': '骷髅剑客',
    'Skeleton Knight': '骷髅骑士',
    'Skeleton Mage': '骷髅法师',
    'Skeleton Pirate': '骷髅海盗',
    'Skeleton Samurai': '骷髅武士',
    'Skeleton Warrior': '骷髅战士',
    'Zaart, the Dominator Goblin': '霸主山精',
    'Banshee, Skeleton Lord': '女妖骷髅领主',
    'Molten Spider': '熔岩蛛妖',
    'Cerberus Ptolemaios': '三头犬',
    'Hellhound Inferni': '幽冥犬',
    'Berthelot, the Undead King': '亡灵之王',
    'Slime King': '粘液王',
    'Zodiac Cancer': '巨蟹星君',
    'Alfadriel, the Light Titan': '光耀斑点，真修',
    'Tiamat, the Dragon Knight': '红龙骑者',
    'Nameless Fallen King': '无名沉沦帝君',
    'Zodiac Aries': '白羊星君',
    'Llyrrad, the Ant Queen': '军星蚁主',
    'Clockwork Spider': '机关蛛妖',
    'Aragorn, the Lethal Wolf': '究极狼王',
    'Naizicher, the Spider Dragon': '蛛龙',
    'Ulliot, the Deathlord': '死灵司命',
    'Ifrit': '火焰之灵',
    'Shiva': '寒冰之灵',
    'Behemoth': '移山巨兽',
    'Blood Manipulation Feral': '血脉寄藏魔',
    'Thanatos': '死灵司神',
    'Darkness Angel Reaper': '黑暗收割者',
    'Zalaras, the Dragon Emperor': '龙帝',
    ...
    'Dark Knight': '暗黑骑士',
    'Phantom Assassin': '幽灵刺客',
    'Wraith': '怨灵',
    'Cursed Mummy': '咒者',
    'Bone Dragon': '骸骨龙',
    'Void Demon': '虚空魔',
    'Ancient Guardian': '上古苏醒者'
};
...
const generateRandomEnemy = (condition) => {
    ...
    const enemyNames = [
        ...
        // News1
        'Golem','Fire Elemental', 'Ice Elemental', 'Earth Elemental','Spirit of the Forest', 'Dark Knight', 'Phantom Assassin','Wraith', 'Cursed Mummy', 'Bone Dragon', 'Void Demon', 'Ancient Guardian'
    ];
        ...
        case "Offensive":
            ...
            if (condition == "guardian") {
                ...
                selectedEnemies = enemyNames.filter(name => [
                    ...
                    'Dark Knight', 'Phantom Assassin','Wraith', 'Cursed Mummy'
                ].includes(name));
            }
        ...
        case "Balanced":
            ...
            if (condition == "guardian") {
                ...
                selectedEnemies = enemyNames.filter(name => [
                    ...
                    'Spider',
                    'Skeleton Knight', 'Skeleton Warrior',
                    'Fire Elemental', 'Ice Elemental', 'Earth Elemental','Spirit of the Forest'
                ].includes(name));
            }
            enemy.name = selectedEnemies[Math.floor(Math.random() * selectedEnemies.length)];
            setEnemyStats(enemy.type, condition);
    ...
    setEnemyImg();
    enemy.name = translationMap[enemy.name];
}
